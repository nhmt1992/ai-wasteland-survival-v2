...
function Get-FreeBackendPort {
  ...
  throw "バックエンド用の空きポートが見つかりませんでした（開始: $PreferredPort）"
}
...
function Format-ShortIdentifier {
  param([string]$Value)
  ...
  if ($Value -match '^[0-9a-fA-F-]{36}$') {
    return "$($Value.Substring(0, 8))…$($Value.Substring($Value.Length - 4, 4))"
  }
...
function New-StatusChip {
  ...
  $label.Font = New-Object System.Drawing.Font('Yu Gothic UI', 9, [System.Drawing.FontStyle]::Bold)
...
function New-InfoCard {
  ...
  $titleLabel.Size = New-Object System.Drawing.Size(240, 18)
  $titleLabel.ForeColor = [System.Drawing.Color]::FromArgb(190, 199, 209)
  $titleLabel.Font = New-Object System.Drawing.Font('Yu Gothic UI', 8.5)
  $titleLabel.Text = $Title

  $valueLabel = New-Object System.Windows.Forms.Label
  $valueLabel.Location = New-Object System.Drawing.Point(14, 34)
  $valueLabel.Size = New-Object System.Drawing.Size(252, 24)
  $valueLabel.AutoEllipsis = $true
  $valueLabel.ForeColor = [System.Drawing.Color]::White
  $valueLabel.Font = New-Object System.Drawing.Font('Yu Gothic UI Semibold', 11)
  $valueLabel.Text = $Value

  $noteLabel = New-Object System.Windows.Forms.Label
  $noteLabel.Location = New-Object System.Drawing.Point(14, 63)
  $noteLabel.Size = New-Object System.Drawing.Size(252, 24)
  $noteLabel.ForeColor = [System.Drawing.Color]::FromArgb(166, 176, 186)
  $noteLabel.Font = New-Object System.Drawing.Font('Yu Gothic UI', 8.5)
  $noteLabel.Text = $Note
...
function Get-ServiceStatusText {
  ...
  switch (Get-ServiceUiState -State $State) {
    'running' { return '実行中' }
    'failed' { return '起動失敗' }
    'exited' { return '終了' }
    default { return '停止中' }
...
function Update-ServiceCard {
  ...
  switch ($stateKind) {
    'running' {
      Set-ChipState -Label $chip -Text '実行中' -BackColor $ThemeSuccess -ForeColor $ThemeText
      $Ui.Start.Enabled = $false
      $Ui.Stop.Enabled = $true
    }
    'failed' {
      Set-ChipState -Label $chip -Text '起動失敗' -BackColor $ThemeDanger -ForeColor $ThemeText
      $Ui.Start.Enabled = $true
      $Ui.Stop.Enabled = $false
    }
    'exited' {
      Set-ChipState -Label $chip -Text '終了' -BackColor $ThemeSurfaceStrong -ForeColor $ThemeText
      $Ui.Start.Enabled = $true
      $Ui.Stop.Enabled = $false
    }
    default {
      Set-ChipState -Label $chip -Text '停止中' -BackColor $ThemeSurfaceStrong -ForeColor $ThemeText
      $Ui.Start.Enabled = $true
      $Ui.Stop.Enabled = $false
    }
  }
  ...
  $Ui.Endpoint.Text = "開く: $($State.Definition.TargetUrl)"
  if ($State.Status -like 'Launch failed*' -and -not [string]::IsNullOrWhiteSpace($State.LastError)) {
    $Ui.Detail.Text = "エラー: $($State.LastError)"
  } elseif ($State.LastStartedAt) {
    $Ui.Detail.Text = "開始時刻: $($State.LastStartedAt.ToString('yyyy-MM-dd HH:mm:ss'))"
  } else {
...
function Refresh-Overview {
  ...
  $Overview.Backend.Note.Text = "バックエンドの待受ポート: $BackendPort"
  $Overview.Running.Value.Text = "$runningCount / $($ServiceState.Count)"
  $Overview.Running.Note.Text = '起動中の開発サービス'
  $Overview.Default.Value.Text = "$defaultStreamerHandle / $(Format-ShortIdentifier $defaultWorldId)"
  $Overview.Default.Note.Text = '既定の配信者 / ワールド'
}

function Resize-ServiceCards {
  param(
    [System.Windows.Forms.FlowLayoutPanel]$PanelHost,
    [hashtable]$ServiceCards
  )

  if ($null -eq $PanelHost -or $null -eq $ServiceCards) {
    return
  }

  $availableWidth = [Math]::Max(760, $PanelHost.ClientSize.Width - $PanelHost.Padding.Left - $PanelHost.Padding.Right - 24)
  $useTwoColumns = $availableWidth -ge 1100
  $cardWidth = if ($useTwoColumns) {
    [Math]::Floor(($availableWidth - 12) / 2)
  } else {
    $availableWidth
  }

  foreach ($cardUi in @($ServiceCards.Values)) {
    if ($null -eq $cardUi) {
      continue
    }
    ...
    if ($useTwoColumns) {
      $cardUi.Card.Margin = New-Object System.Windows.Forms.Padding(0, 0, 12, 12)
    } else {
      $cardUi.Card.Margin = New-Object System.Windows.Forms.Padding(0, 0, 0, 12)
    }

    $cardUi.Card.Width = $cardWidth
  }
}
...
$serviceDefinitions = @(
  New-ServiceDefinition `
    -Key 'backend' `
    -Label 'バックエンド' `
    -Description 'サーバー / 世界状態 / Tick / ギフト処理' `
    -Command 'npm run dev:backend' `
    -WindowTitle 'AI Wasteland Survival v2 - バックエンド' `
  ...
  New-ServiceDefinition `
    -Key 'game' `
    -Label 'ゲーム画面' `
    -Description '配信者向け 2.5D ゲーム画面' `
    -Command 'npm run dev:game' `
    -WindowTitle 'AI Wasteland Survival v2 - ゲーム画面' `
    -WorkspacePath $repoRoot `
  ...
  New-ServiceDefinition `
    -Key 'streamer' `
    -Label '配信者コンソール' `
    -Description '配信者の操作と監視' `
    -Command 'npm run dev:streamer' `
    -WindowTitle 'AI Wasteland Survival v2 - 配信者コンソール' `
    -WorkspacePath $repoRoot `
  ...
  New-ServiceDefinition `
    -Key 'overlay' `
    -Label 'オーバーレイ' `
    -Description 'OBS 用の表示レイヤー' `
    -Command 'npm run dev:overlay' `
    -WindowTitle 'AI Wasteland Survival v2 - オーバーレイ' `
    -WorkspacePath $repoRoot `
  ...
  New-ServiceDefinition `
    -Key 'viewer' `
    -Label '視聴者ページ' `
    -Description '視聴者の作成・閲覧ページ' `
    -Command 'npm run dev:viewer' `
    -WindowTitle 'AI Wasteland Survival v2 - 視聴者ページ' `
    -WorkspacePath $repoRoot `
  ...
  New-ServiceDefinition `
    -Key 'admin' `
    -Label '管理画面' `
    -Description 'プラットフォーム管理' `
    -Command 'npm run dev:admin' `
    -WindowTitle 'AI Wasteland Survival v2 - 管理画面' `
    -WorkspacePath $repoRoot `
...
function Start-ServiceWindow {
  ...
  try {
    ...
    Show-Message -Text "起動できませんでした: $($state.Definition.Label)`n$($state.LastError)" -Title '起動失敗' -Icon Error
  }
...
function Create-ServiceCard {
  ...
  $panel.Size = New-Object System.Drawing.Size(760, 176)
  ...
  $title.Font = New-Object System.Drawing.Font('Yu Gothic UI Semibold', 12)
  ...
  $detail.Font = New-Object System.Drawing.Font('Yu Gothic UI', 8.75)
  ...
  $endpoint.Font = New-Object System.Drawing.Font('Yu Gothic UI', 8.5)
  $endpoint.Text = "開く: $($Definition.TargetUrl)"
  ...
  $startButton = New-ActionButton -Text '起動'
  $stopButton = New-ActionButton -Text '停止'
  $openButton = New-ActionButton -Text '開く'
...
$form.Text = 'AI Wasteland Survival v2 - 開発ランチャー'
...
$subtitle.Font = New-Object System.Drawing.Font('Yu Gothic UI', 9.5)
$subtitle.ForeColor = $theme.Muted
$subtitle.Text = '単一配信者向けの起動パネル。バックエンドの待受先とフロントエンドのプロキシを自動でそろえます。'
...
$openReadmeButton = New-ActionButton -Text 'README を開く'
$openRunbookButton = New-ActionButton -Text 'Runbook を開く'
...
$overviewCards = @{
  Backend = New-InfoCard -Title 'バックエンド先' -Value $backendTarget -Note '空きポートを自動選択します'
  Running = New-InfoCard -Title '稼働中サービス' -Value '0 / 0' -Note '状態は自動更新されます'
  Default = New-InfoCard -Title '既定の配信者 / 世界' -Value "$defaultStreamerHandle / $(Format-ShortIdentifier $defaultWorldId)" -Note 'ゲーム画面と視聴者ページの既定値です'
}
...
$startAllButton = New-ActionButton -Text 'すべて起動'
$stopAllButton = New-ActionButton -Text 'すべて停止'
$refreshButton = New-ActionButton -Text '更新'
$openBackendButton = New-ActionButton -Text 'バックエンド'
$openGameButton = New-ActionButton -Text 'ゲーム画面'
$openViewerButton = New-ActionButton -Text '視聴者'
...
$serviceHost.FlowDirection = 'LeftToRight'
$serviceHost.WrapContents = $true
...
function Refresh-AllUi {
  ...
  $footerLabel.Text = "リポジトリ: $repoRoot  |  稼働中: $runningCount / $($serviceDefinitions.Count)"
  $footerHint.Text = "バックエンド先: $backendTarget"
}
...
$serviceHost.Add_SizeChanged({
  Resize-ServiceCards -PanelHost $serviceHost -ServiceCards $ui.Services
})
...
$form.Add_FormClosing({
  ...
  $result = [System.Windows.Forms.MessageBox]::Show(
    '起動中のサービスがあります。ランチャーを終了する前に、起動済みのウィンドウも閉じますか？',
    '終了の確認',
